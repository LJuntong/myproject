...
from myadmin.views.userviews import uploads
...
def grzx(request):
    ...
    if request.method == 'GET':
        ...
        context = {'uinfo':ob}
        # 显示编辑页面
        return render(request,'myhome/grzx.html',context)
        print('aaaaaa')
        ...
        # 判断是否上传了新的图片
            ...
            request.session['VipUser'] = {'uid':ob.id,'username':ob.username}
            # print('qqqqqq')
            return HttpResponse('<script>alert("修改完成");location.href="'+reverse('myhome_mycenter')+'"</script>')
        except:
            return HttpResponse('<script>alert("更新失败");location.href="'+reverse('myhome_grzx')+'?uid='+str(ob.id)+'"</script>')


    # return render(request,'myhome/grzx.html')